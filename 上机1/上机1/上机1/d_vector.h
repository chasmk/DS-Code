	...
	vSize(0), vCapacity(0), vArr(NULL)
{
	...
	for (i = 0; i < vSize; i++)
		vArr[i] = T();//括号里面可以填数字,默认0 预标准 C++ 形式强制类型转换
}
...
template <typename T>
void miniVector<T>::delete_unique()
{
	int end = vSize - 1;
	while (end != 0)
	{
	...
	for (; cur < vSize;)
	{
		T tmp = vArr[cur++];
		vArr[p++] = tmp;
		while (vArr[cur] == tmp&&cur<vSize) cur++;
	}
	vSize = p;
}

template <typename T>
void miniVector<T>::unique()
{
	int* flag = new int[vSize];
	for (int i = 0; i < vSize; i++) flag[i] = 0;

	for (int i = 0; i < vSize;i++ )
	{
		while (flag[i] == 1 && i < vSize) i++; //i指向第一个未重复的元素位置
		for (int j = i + 1; j < vSize; j++)
		{
			if (vArr[j] == vArr[i])flag[j] = 1;//1代表是重复元素
		}
	}
	int p = 0;
	for (int i = 0; i < vSize; i++)
	{
		if (flag[i] == 0) vArr[p++] = vArr[i];
	}
	vSize = p;
	delete[] flag;
}
template <typename T>
void miniVector<T>::unique_new()
{
	int flag[65535] = { 0 };
	miniVector<T> tmp;

	for (int i = 0; i < vSize; i++) flag[(vArr+i)]++;
	int cur = 0;
	for (int i = 0; i < vSize; i++)
	{
		if (flag[vArr[i]] == 1)
